...
class NAO_RedTeam_Striker(Robot):
    ...
    def position_refresh(self):
        if self.receiver.getQueueLength() > 0:
            ...
            robot_position = np.float64(shared_info["RedTeam_DefenderLeft"]["position"])
            robot_orientation = np.float64(shared_info["RedTeam_DefenderLeft"]["orientation"])
    ...
    def trackingBall(self):
        epsilon_ = 0.2
        robot_position = self.__shared_info["RedTeam_DefenderLeft"]["position"]
        football_position = self.__shared_info["football"]["position"]
        orientation = self.__shared_info["RedTeam_DefenderLeft"]["orientation"]
